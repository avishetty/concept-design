// Onboarding scenario — redesigned for a calm, modern chat.
// No boxes-within-boxes: Walt speaks like a person, choices are inline pills,
// completed work is a single tappable row that opens a right-hand disclosure.

function OnboardingScenarioScreen() {
...
// Projects landing — full-bleed overlay shown when the user clicks the platform name
// in the chat header. Lists all platforms with their source/target/config summary.
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, background: 'var(--bg-app)' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '28px 28px 24px' }}>
        <div style={{ maxWidth: 720, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 28 }}>
          ...
          {/* Walt opens — one avatar, multiple lines stack under it */}
          <WaltStack>
          ...
          <WaltStack silent>
            ...
            <ArtifactRow
              kind="plan"
              title="Plan ready — 6 steps"
              sub="bronze → silver (review) → staging/git → gold → semantic"
              onOpen={() => open('plan')}
              active={openArtifact === 'plan'}
            />
          </WaltStack>

          <UserBubble>Start with bronze. I'll wire GitHub after silver lands.</UserBubble>

          {/* Bronze — Walt working, then stacks of status rows */}
          <WaltStack working>
            ...
            <ArtifactRow
              kind="bronze"
              title="Bronze layer · 220 tables"
              sub="Click to inspect models, tests, and run log"
              onOpen={() => open('bronze')}
              active={openArtifact === 'bronze'}
            />
          </WaltStack>

          {/* Silver — needs decision */}
          <WaltStack working>
          ...
          <WaltStack silent>
            ...
            <ArtifactRow
              kind="silver"
              title="Silver ready · open PR or download"
              sub="Conformed entities · semantic-stable"
              onOpen={() => open('silver')}
              active={openArtifact === 'silver'}
            />
          </WaltStack>
          ...
          <WaltStack silent>
            ...
            <ArtifactRow
              kind="semantic"
              title="Semantic model · finance.yml"
              sub="Measures, dimensions, relationships — ready for review"
              onOpen={() => open('semantic')}
              active={openArtifact === 'semantic'}
            />
          </WaltStack>
      ...
      {/* composer — richer input box, matches engineer surface */}
...
// ─────────── chat primitives — flat, no nested cards ───────────

// One Walt avatar per cluster of his messages; child BotLines stack flush-left.
// `silent` hides the avatar (used after the first appearance so we don't repeat Walt visually).
// `working` shows Walt's eyes scanning left-right — used while work is in progress.
...
function UserBubble({ children }) {
  // user reply — right-aligned pill, no avatar.
  return (
    <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
      <div style={{
        maxWidth: 520, padding: '9px 14px', borderRadius: 16,
...
function PillRow({ values, choice }) {
  // Picked options align right under the user's reply (treated as part of their answer).
  // Choice options stay on the left under Walt's question.
  return (
...
function TargetPills({ selected }) {
  ...
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, justifyContent: 'flex-end' }}>
      {list.map(l => {
        const on = l.toLowerCase() === selected;
        if (!on) return null;
        return (
          <span key={l} style={{
            display: 'inline-flex', alignItems: 'center', gap: 6,
...
function KVInline({ pairs }) {
  // inline key/value confirmations under a Walt message — no extra card, just a thin list
  return (
...
function ArtifactRow({ kind, title, sub, onOpen, active }) {
  // The single, clickable disclosure trigger.  Compact pill, opens right panel.
  ...
  return (
    ...
    >
      <span style={{
        ...
        background: `${tintByKind[kind]}1A`,
        color: tintByKind[kind],
...
// ─────────── right-hand disclosure ───────────

function OnboardingDisclosure({ which, tab, setTab, onClose }) {
  // Disclosure has tabs (Plan / Workspace / Context) at the top so you can jump
  // anywhere — clicking an inline ArtifactRow just defaults the tab.
  const tabs = [
  ...
  return (
    <div style={{ width: 480, flexShrink: 0, background: 'var(--bg-surface)', borderLeft: '1px solid var(--border-subtle)', display: 'flex', flexDirection: 'column', minHeight: 0 }}>
      ...
      {/* Sub-header showing which item is in focus (when not on plan) */}
      {tab !== 'plan' && (
...
Object.assign(window, { OnboardingScenarioScreen });
