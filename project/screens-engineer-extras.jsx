// Supporting engineer surfaces, plus the foundations + landing artboards.
// Each is a self-contained component rendered inside a WaltWindow.

// ─────────────────────────────────────────────────────────────
//  PIPELINE DAG
// ─────────────────────────────────────────────────────────────
function PipelineDAGScreen() {
  return (
...
function SubHeader({ title, sub, actions = [], left }) {
  return (
    <div style={{
...
function DAGCanvas() {
  // hand-laid DAG positions on 1100x540 canvas
  const nodes = [
    // sources (col 0)
...
function SQLEditor() {
  // editor with one inline AI ghost suggestion
  const lines = [
...
function RunBadge({ state }) {
  // Single neutral chip — the dot color encodes state. Keeps the runs table calm
  // and matches the chat / plan visual vocabulary.
...
Object.assign(window, {
  PipelineDAGScreen, MedallionScreen, SQLEditorScreen, CatalogScreen, ObservabilityScreen,
  SubHeader,
});
